...
function stopActiveAudio() {
  ...
  voiceStatus.classList.remove('playing');
}

async function speakText(text) {
  if (!currentState.speechEnabled || !currentState.hasFishApiKey) {
    return;
  }
  ...
  try {
    ...
    if (requestId === speechRequestId) {
      voiceStatus.classList.remove('loading', 'playing');
      pet.classList.remove('talking');
      showMessage(error.message || '语音暂时不可用', 3200, false);
    }
  }
}
...
function playReaction(forcedType) {
  ...
  setAction(reaction.action, 1250);
